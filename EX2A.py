...
def entropy(col):
    ...
    for count in counts.values():
        ...
        ent -= p * math.log2(p)
    return ent
def gain_ratio(data, col_idx, target_idx):
    ...
    for v in vals:
        ...
        weighted += ratio * entropy(subset)
        split_info -= ratio * math.log2(ratio)
    if split_info == 0:
        return 0
    return (total_entropy - weighted) / split_info
def build_tree(data, attributes, target_idx):
    ...
    gains = [gain_ratio(data, i, target_idx) for i in range(len(attributes))]
...
tree_c45 = build_tree(data, attributes[:-1], target_idx)
